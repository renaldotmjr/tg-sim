...
int CONF_TRIPLE(char _print_mtx, char _print_micro,  char orbital_1, int no_e_1, char orbital_2, int no_e_2, char orbital_3, int no_e_3) {
    
    int i = 0, ii = 0, iii = 0;      // Contadores 'for'
    ...
    double NmeT_T = 0.0,       // N˙mero de Microestados da configuraÁ„o completa
    NmeT_1 = 0.0,       // N˙mero de Microestados da configuracao do primeiro orbital
    NmeT_2 = 0.0,       // N˙mero de Microestados da configuracao do segundo orbital
    NmeT_3 = 0.0;       // N˙mero de Microestados da configuracao do terceiro orbital
    ...
    if( no_e_1 <= ORB) {
        ...
        // Matrizes de ml e ms dos orbitais mistos
        ...
        if( no_e_2 <= ORB) {
            ...
            //printf("\nMTX_MS_1 ; MTX_ML_1");
            //for (i = 0; i < NmeT_1; i++){
            //    printf("\nMs: %d    Ml: %d", MTX_MS_1[i], MTX_ML_1[i]);
            //}
            
            // Impressao do NmeT na tela
            ...
            ///// ALTERACAO DO ALGORITMO - UTILIZANDO POUCA MEMORIA PARA ORBITAIS MISTOS
            ...
            // Trabalhando o terceiro orbital
            ...
            // Testando o numero de elÈtrons
	           if( no_e_3 <= ORB) {
                   ...
                   F_GENMICRO(_print_micro, ORB, i, no_e_3, MTX0_ms, MTX0_ml );
                   
                   ///// ALTERA«√O DO ALGORITMO - UTILIZANDO POUCA MEMORIA PARA ORBITAIS MISTOS
                   
                   // Gera matriz MTX_MS_T e MTX_ML_T pela combinacao das MTX_MS_X e MTX_ML_X
                   ...
                   for(i = 0; i < NmeT_1; i++) {
                       for(ii = 0; ii < NmeT_2; ii++) {
                           for(iii = 0; iii < NmeT_3; iii++) {
                               Ms = MTX_MS_1[i] + MTX_MS_2[ii] + MTX_MS_3[iii];
                               Ml = MTX_ML_1[i] + MTX_ML_2[ii] + MTX_ML_3[iii];
                               
                               //printf("\nMs: %d (%d + %d + %d) Ml: %d (%d + %d + %d)", Ms, MTX_MS_1[i], MTX_MS_2[ii], MTX_MS_3[iii], Ml, MTX_ML_1[i], MTX_ML_2[ii], MTX_ML_3[iii]);
                   ...
                   printf("\n\nThe electronic configuration %c%d%c%d%c%d has %0.1f microstates\n",
                          orbital_1, no_e_1, orbital_2, no_e_2, orbital_3, no_e_3, NmeT_1*NmeT_2*NmeT_3);
               ...
               } //fim do 'if' que testa o n˙mero de elÈtrons do terceiro orbital
        ...
        printf("\nNumber of electrons for %c, %c and %c orbitals: %d, %d and %d.\nAre you sure?\nChoose a valid electronic configuration or go work with human sciences or biology...\n",
               orbital_1, orbital_2, orbital_3, no_e_1, no_e_2, no_e_3);
...
// Fim da selecao de tres orbitais